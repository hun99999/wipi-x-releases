...
**0.1.11 비공개 테스트(Alpha)를 2026년 9월 13일 심사 요청했습니다.** Play 제공은 대기 중입니다. 현재 제공이 확인된 버전은 **0.1.10**이며, 아래 참여 절차로 설치한 뒤 새 버전 반영 후 업데이트해 주세요.
...
- 현재 제공 확인 버전은 0.1.10이며 0.1.11은 제공 대기 중입니다. 공식 설치와 업데이트는 Google Play에서 제공합니다.
- 앱에는 게임 파일이 포함되어 있지 않습니다. 이용 권한이 있는 파일을 준비해 주세요.
- 초기 테스트 목록은 [31개(KTF 18개 · LGT 13개)](COMPATIBILITY.md)입니다. 0.1.11은 정확히 검증한 축구·야구·메이플 도적편·레전드·붕어빵3 5개를 지원됨으로 표시하며, 나머지는 검증 대기를 유지합니다. 게임별 범위는 [이번 릴리즈 노트](UPDATES.md)를 확인하세요.
...
**0.1.11 (Alpha)** was submitted for review on September 13, 2026, and Play availability is pending. **0.1.10** remains the version confirmed available; update after the new build reaches Play. Complete the group membership and Play opt-in steps below before installing through Google Play.
...
Joining the group does not automatically opt you into the Play test. The initial test is configured for users aged 18 or older in South Korea, on Android 8.0 or later. The app does not include game files. The [31-game roster](COMPATIBILITY.md) records the initial selection. Version 0.1.11 marks the exact verified files for 2010 Super Soccer (LGT), 2010 Pro Baseball (LGT), MapleStory: Thief Edition (KTF), Legend of Master (LGT), and Bungeoppang Tycoon 3 (KTF) as Supported; other final decisions remain pending. See the [release notes](UPDATES.md) for scope.
