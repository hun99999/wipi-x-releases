...
이 문서는 0.1.22 테스트 앱의 사용 흐름을 설명합니다. **0.1.22 (Alpha)**는 2026년 9월 15일 심사 전송을 요청했습니다. Play 자동 검사 진행 중이며 제공은 대기 중입니다. 현재 제공이 확인된 버전은 **0.1.11**입니다. Android 8.0 이상이 필요하며, 앱 설치와 업데이트는 Google Play에서 제공합니다. [테스트 참여 안내](TESTING.md)에 따라 그룹 가입과 Play 테스트 참여를 마친 뒤 설치하세요.
...
This guide describes 0.1.22 (Alpha). **0.1.22 (Alpha)** was submitted for review on September 15, 2026. Play automated checks are running and availability is pending. **0.1.11** is currently confirmed available. Follow the [testing guide](TESTING.md#english), join the testers group, opt in, and install through Google Play. Android 8.0 or later is required.
