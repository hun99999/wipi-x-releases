...
이 문서는 0.1.28 테스트 앱의 사용 흐름을 설명합니다. **0.1.28 (Alpha)**는 심사 요청을 완료했으며 Play 제공을 기다리고 있습니다. 현재 제공이 확인된 버전은 **0.1.22**입니다. Android 8.0 이상이 필요하며, 앱 설치와 업데이트는 Google Play에서 제공합니다. [테스트 참여 안내](TESTING.md)에 따라 그룹 가입과 Play 테스트 참여를 마친 뒤 설치하세요.
...
This guide describes 0.1.28 (Alpha). **0.1.28 (Alpha)** has been submitted for review and is awaiting Play availability. **0.1.22** is currently confirmed available. Follow the [testing guide](TESTING.md#english), join the testers group, opt in, and install through Google Play. Android 8.0 or later is required.
