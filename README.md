...
> **최신 기록 · 0.1.35 Alpha / Play 제공 대기** — [WIPI-X 0.1.35 — 미니게임천국 2·이노티아 2 지원 목록 반영](https://github.com/hun99999/wipi-x-releases/releases/tag/v0.1.35) · [전체 릴리즈 보기](https://github.com/hun99999/wipi-x-releases/releases)
...
**WIPI-X 앱의 공식 설치와 업데이트는 Google Play에서만 제공합니다.** **0.1.35 (Alpha)**는 심사 요청을 완료했으며 Play 제공을 기다리고 있습니다. 현재 제공이 확인된 버전은 **0.1.33**입니다. 기존 앱을 삭제하지 않고 Google Play에서 업데이트할 수 있습니다.
...
- **0.1.35 지원 목록 반영:** 미니게임천국 2(KTF)와 이노티아 연대기 2(KTF)의 검증된 파일을 지원됨으로 표시합니다. 지원 파일은 14개이며 기존 저장·설정·게임 ZIP을 유지합니다.
...
게임은 오프라인으로 실행하며 로그인·광고·인앱 결제 없이 동작합니다. 앱 시작과 수동 업데이트 확인에는 GitHub 공개 릴리스 조회를 위한 인터넷 연결을 사용합니다. 기기와 게임에 따라 실행·저장 범위와 배속 성능은 다를 수 있습니다. [0.1.35 변경 안내](docs/UPDATES.md)에서 이번 테스트 버전의 내용을 확인할 수 있습니다.
...
The app is distributed **exclusively through Google Play**. **0.1.35 (Alpha)** has been submitted for review and is awaiting Play availability. **0.1.33** is currently confirmed available. Join the testers group and opt in using the same Google account. See the [testing guide](docs/TESTING.md#english) and [release notes](https://github.com/hun99999/wipi-x-releases/releases).

Version 0.1.35 marks the verified files for MiniGame Paradise 2 (KTF) and Inotia Chronicles 2 (KTF) as Supported, bringing the total to fourteen. Existing saves, settings and game ZIPs remain compatible.
...
This repository contains project information, usage guides, compatibility notes, and user support. It does not distribute app source code, APK/AAB packages, or game files. The initial roster contains **31 entries (18 KTF and 13 LGT)**, including Rhythm Star, 2010 Pro Baseball, and Tempest. Version 0.1.35 marks the exact verified files for 2010 Super Soccer (LGT), 2010 Pro Baseball (LGT), MapleStory: Thief Edition (KTF), Legend of Master (LGT), Bungeoppang Tycoon 3 (KTF), Inotia Chronicles (KTF), Black Room 2 (LGT), Oceanus (LGT), Wild Frontier 1 (LGT), TalesWeaver: Ispin, Maximin and Lucian (LGT), MiniGame Paradise 2 (KTF), and Inotia Chronicles 2 (KTF) as Supported. Other final decisions remain pending, and the roster will expand as more games are checked.
